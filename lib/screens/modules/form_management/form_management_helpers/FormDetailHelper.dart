...
import 'form_dialogs/FormDialogs.dart';
import 'form_question_management/QuestionSelectionDialog.dart';
import 'QuestionCreationManager.dart'; // Import the QuestionCreationManager
...
class _FormDetailScreenState extends State<FormDetailHelper> {
  ...
  Future<void> _fetchFormDetails() async {
    ...
    try {
      final details = await _formApiService.fetchFormById(context, widget.form['id']);
      if (mounted) {
        setState(() {
          formDetails = details;
          isLoading = false;
    ...
    QuestionCreationManager().showQuestionTypeSelection(
      ...
      questionTypes,
        ...
        setState(() {
          ...
          if (_isValidating) {
            _questionCreationValidStates.last = _validateQuestionCreation(_questionCreations.length - 1);
          }
        });
      },
    );
  ...
  bool _validateQuestionCreation(int index) {
    final data = _questionCreations[index];
    return data.questionTextController.text.isNotEmpty && data.selectedQuestionTypeId != null;
  }
                  ...
                  child: isLoading
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if ((formDetails?['questions'] as List? ?? []).isEmpty)
                          const Center(
                            child: Padding(
                              padding: EdgeInsets.all(16.0),
                              child: Text(
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (showMenuButtons || orientation == Orientation.landscape)
                        Column(
                          children: [
                            const SizedBox(height: 8),
                            FloatingActionButton(
                            ...
                            FloatingActionButton(
                              heroTag: 'export_form',
                              onPressed: _showExportDialog,
                              backgroundColor: const Color.fromARGB(255, 74, 180, 246),
                              child: const Icon(
                                Icons.ios_share_rounded,
                                color: Colors.white,
                              ),
...
class _FormDetailScreenState extends State<FormDetailHelper> {
  ...
  void _deleteFormQuestion(BuildContext context, int formQuestionId) async {
    try {
      final bool? shouldDelete = await FormDialogs.showDeleteQuestionDialog(context);
  ...
  bool _shouldShowAnswerSelection(String questionType) {
    return !['date', 'datetime', 'text', 'user'].contains(questionType.toLowerCase());
  }
